...
class EditLineupScreen extends StatefulWidget {
  ...
  EditLineupScreen({required this.lineup});

  @override
  _EditLineupScreenState createState() => _EditLineupScreenState();
}
...
class _EditLineupScreenState extends State<EditLineupScreen> {
  ...
  void initState() {
    super.initState();
    _selectedArtist = widget.lineup.artistId;
    _selectedStage = widget.lineup.stageId;

    final dateTime = widget.lineup.time;
  ...
  void _saveChanges() async {
    ...
    try {
      await Provider.of<LineupProvider>(context, listen: false).updateLineup(
          widget.lineup.id,
          _selectedArtist!,
          _selectedStage!,
          selectedDateTime);
      ...
      // Handle errors from updateLineup
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Fout bij het bijwerken van de line-up: $error')),
      );
    }
  }
